...
static void my_A_Star_with_set(std::array<std::array<Node, (Y_MAX / Y_STEP)>, (X_MAX / X_STEP)>& mapka, Node& current_Node, Node& destination_Node, sf::RenderWindow& window) {
	...
	for (int i = 0; i < X_MAX / X_STEP; i++) {
		for (int j = 0; j < Y_MAX / Y_STEP; j++) {
			...
			mapka[i][j].hCost = FLT_MAX;//my_calcutateH(mapka[i][j], destination_Node); //zmienilem z FLT_MAX
			mapka[i][j].parentX = -1; //bylo i
			mapka[i][j].parentY = -1; //bylo j
			mapka[i][j].x = i;
			mapka[i][j].y = j;
	...
	std::set<Node, bool(*)(const Node&, const Node&)> openList_set(compareNode);

	//std::vector<Node> openList;
	//std::cout << "rozmiar openList przed emplace = " << openList.size() << std::endl;
	std::cout << "rozmiar openList_set przed emplace = " << openList_set.size() << std::endl;
	//openList.emplace_back(mapka[i][j]);
	openList_set.insert(mapka[i][j]);
	//std::cout << "rozmiar openList po emplace = " << openList.size() << std::endl;
	std::cout << "rozmiar openList_set po emplace = " << openList_set.size() << std::endl;
	bool destination_found = false;
	
	while (!openList_set.empty() && openList_set.size() < (X_MAX / X_STEP)*(Y_MAX / Y_STEP)) { //bylo (!openList.empty() && openList.size() < (X_MAX / X_STEP)*(Y_MAX / Y_STEP))
		std::cout << "wewnatrz glownej petli !openList.empty() && openList.size()" << std::endl;
		
		Node node = *openList_set.begin();
		openList_set.erase(node);
				...
				if (my_isValid_xy(i + newX, j + newY, mapka)) {
					...
					if (my_isDestination(mapka[i + newX][j + newY], destination_Node)) {
						...
						if (mapka[i + newX][j + newY].fCost == FLT_MAX || mapka[i + newX][j + newY].fCost > fNew) {
							...
							openList_set.insert(mapka[i + newX][j + newY]);
						}
					}
				}
			}
...
static void my_A_Star_moja_rzezba(std::array<std::array<Node, (Y_MAX / Y_STEP)>, (X_MAX / X_STEP)>& mapka, Node& current_Node, Node& destination_Node, sf::RenderWindow& window) {
	//std::cout << "test" << std::endl;
	std::vector<Node> empty;// return empty;
	if (my_isValid(destination_Node) == false) {
		std::cout << "destination is an obstacle, returning empty path" << std::endl;
